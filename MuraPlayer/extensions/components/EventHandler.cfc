...
component extends="mura.plugin.pluginGenericEventHandler" accessors=true output=false {
	...
	public any function onPageMuraPlayerBodyRender(required struct $) output=false {
		...
		// Image
		local.image = $.getURLForImage(fileid=$.content('muraPlayerPlaceholder'),width=local.width,height=local.height);
		...
		local.player = dspJWPlayer(
			...
			,controlbardisplay = $.content('muraPlayerControlbarDisplay')
	...
	*/
			...
			while ( local.it.hasNext() ) {
				...
				if ( !Len(Trim(arguments.listContentIDs)) || ( Len(Trim(arguments.listContentIDs)) && ListFindNoCase(arguments.listContentIDs, local.item.getValue('contentid')) ) ) {
					...
					local.image = $.getURLForImage(
						fileid=local.item.getValue('muraPlayerPlaceholder')
	...
	private void function setPlayerDimensionsOptionList() output=false {
		variables.playerDimensionsOptionList = 'default^320x240^480x360^640x480^400x225^480x272^640x360^720x405^960x540^1280x720^1920x1080^516x290';
	}
	...
	private void function setPlayerDimensionsOptionLabelList() output=false {
		variables.playerDimensionsOptionLabelList = 'Use Default Dimensions From Site Settings^320x240 (4:3)^480x360 (4:3)^640x480 (4:3)^400x225 (16:9)^480x272 (16:9)^640x360 (16:9)^720x405 (16:9)^960x540 (16:9)^1280x720 (16:9)^1920x1080 (16:9)^516x290 (16:9)';
	}
	...
	private void function setPlayerSkinsOptionList() output=false {
		...
		variables.playerSkinsOptionList = REReplaceNoCase(
			ArrayToList(DirectoryList(local.path,false,'name','*.xml','directory ASC'),'^')
			, '.xml'
			, ''
			, 'ALL'
		);
